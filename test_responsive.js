...
function testResponsiveLayout() {
  ...
  console.log(hasMediaQuery ? '✅ Media query found' : '❌ Media query missing');
  ...
  const mobileRules = [
    ...
    { rule: '.round-column', property: 'display: contents', hasRule: html.includes('display: contents') }
  ];

  mobileRules.forEach(({ rule, property, hasRule }) => {
    console.log(hasRule ? `✅ ${rule} { ${property} }` : `❌ ${rule} { ${property} } missing`);
  });
  ...
  console.log(hasMobileFunc ? '✅ isMobile() function exists' : '❌ isMobile() function missing');
  console.log(checksMobileInRender ? '✅ render() checks isMobile()' : '❌ render() does not check isMobile()');
  ...
  console.log(hasTopOverride ? '✅ top property reset on mobile' : '❌ top property not reset');
  console.log(hasTransformOverride ? '✅ transform property reset on mobile' : '❌ transform not reset');

  // Test 5: Check viewport meta tag
  console.log('\nTest 5: Viewport Configuration');
  const hasViewportMeta = html.includes('viewport');
  console.log(hasViewportMeta ? '✅ Viewport meta tag present' : '❌ Viewport meta tag missing');
